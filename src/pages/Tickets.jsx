...
const COLUMNAS = [
  { id: 'backlog',    label: 'Backlog',       color: 'var(--app-text-subtle)' },
...
const PRIORIDADES = [
  ...
  { key: 'baja',  label: 'Baja',  color: 'var(--app-text-subtle)', bg: '#f3f4f6' },
];
...
export default function Tickets() {
  ...
  const inp = (extra = {}) => ({ width: '100%', border: '1px solid var(--app-border)', borderRadius: 8, padding: '9px 12px', fontSize: 13, outline: 'none', background: 'var(--app-surface)', boxSizing: 'border-box', color: 'var(--app-text)', ...extra });
  ...
  return (
    <div style={{ background: 'var(--app-bg)', minHeight: '100%', display: 'flex', flexDirection: 'column' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px 16px', background: 'var(--app-surface)', borderBottom: '1px solid var(--app-border)', position: 'sticky', top: 0, zIndex: 20 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--app-text)', margin: 0 }}>Tickets</h1>
          ...
          return (
            <div key={col.id}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, minHeight: 80, background: isDragOver ? 'rgba(229,83,83,0.04)' : 'transparent', borderRadius: 10, transition: 'background 0.15s', padding: isDragOver ? '6px' : '0' }}>
                {colTickets.map(ticket => {
                  ...
                  return (
                    <div key={ticket.id}
                      ...
                      style={{ background: 'var(--app-surface)', borderRadius: 10, boxShadow: draggingId === ticket.id ? '0 4px 16px rgba(0,0,0,0.12)' : '0 1px 3px rgba(0,0,0,0.06)', border: '1px solid var(--app-border)', borderLeft: `4px solid ${ticket.color}`, opacity: draggingId === ticket.id ? 0.5 : 1, cursor: 'grab', transition: 'box-shadow 0.15s, opacity 0.15s' }}>
                      {/* Card main row */}
                      <div style={{ padding: '10px 12px 8px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap', marginBottom: asignado || vence ? 7 : 0 }}>
                          ...
                          <span style={{ fontSize: 10, fontWeight: 500, padding: '2px 7px', borderRadius: 4, background: 'var(--app-surface-2)', color: 'var(--app-text-muted)' }}>{ticket.tipo}</span>
                          {vence && (
                      ...
                      <div onClick={() => setExpandedId(isExpanded ? null : ticket.id)}
                        style={{ borderTop: '1px solid var(--app-border-light)', padding: '4px 12px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      {isExpanded && (
                        <div style={{ padding: '8px 12px 10px', borderTop: '1px solid var(--app-border-light)' }}>
                          {ticket.descripcion && (
                            <p style={{ fontSize: 12, color: 'var(--app-text-muted)', lineHeight: 1.5, margin: '0 0 10px' }}>{ticket.descripcion}</p>
                          )}
                ...
                {colTickets.length === 0 && (
                  <div style={{ textAlign: 'center', padding: '20px 10px', color: '#d1d5db', fontSize: 12, border: '1px dashed #e8e8ee', borderRadius: 8, background: 'var(--app-surface)' }}>
                    Sin tickets
                  </div>
                )}
              </div>
        ...
        <div onClick={e => { if (e.target === e.currentTarget) setShowModal(false); }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: 'var(--app-surface)', borderRadius: 14, width: '100%', maxWidth: 540, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: '1px solid var(--app-border-light)' }}>
              <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--app-text)' }}>{editandoId ? 'Editar Ticket' : 'Nuevo Ticket'}</span>
            ...
            <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={{ display: 'block', fontSize: 13, color: 'var(--app-text-2)', marginBottom: 6 }}>Título</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 13, color: 'var(--app-text-2)', marginBottom: 6 }}>Descripción</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 13, color: 'var(--app-text-2)', marginBottom: 6 }}>Prioridad</label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 13, color: 'var(--app-text-2)', marginBottom: 6 }}>Columna</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 13, color: 'var(--app-text-2)', marginBottom: 6 }}>Tipo</label>
                  ...
                  <div style={{ marginTop: 6 }}>
                    {showTipoInput ? (
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        <button onClick={() => setShowTipoInput(false)} style={{ padding: '5px 8px', background: 'none', border: '1px solid var(--app-border)', borderRadius: 6, cursor: 'pointer', fontSize: 11, color: 'var(--app-text-muted)' }}>×</button>
                      </div>
                    ) : (
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', alignItems: 'center' }}>
                        {tipos.filter(t => !TIPOS_DEFAULT.includes(t)).map(t => (
                          <span key={t} style={{ fontSize: 10, background: 'var(--app-surface-2)', border: '1px solid var(--app-border)', borderRadius: 4, padding: '2px 6px', display: 'inline-flex', gap: 3, alignItems: 'center', color: 'var(--app-text-muted)' }}>
                            {t}
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 13, color: 'var(--app-text-2)', marginBottom: 6 }}>Asignado a</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 13, color: 'var(--app-text-2)', marginBottom: 6 }}>Fecha de vencimiento</label>
                  <input type="date" value={form.fechaVence} onChange={e => setForm(p => ({ ...p, fechaVence: e.target.value }))} style={inp()} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 13, color: 'var(--app-text-2)', marginBottom: 6 }}>Color de acento</label>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <input type="color" value={form.color} onChange={e => setForm(p => ({ ...p, color: e.target.value }))} style={{ width: 38, height: 38, border: '1px solid var(--app-border)', borderRadius: 8, cursor: 'pointer', padding: 2 }} />
                    <span style={{ fontSize: 11, color: 'var(--app-text-subtle)' }}>Borde izquierdo de la tarjeta</span>
                  </div>
                </div>
              </div>
            </div>
