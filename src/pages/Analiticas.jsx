    ...
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: 'var(--app-surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--app-text-muted)' }}>{icon}</div>
    ...
    switch (b.tipo) {
      ...
      case 'evento_mas_reg':
        return (
          <div>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--app-text)' }}>{masRegistrosEv?.nombre || '—'}</div>
      ...
      case 'evento_menos_reg':
        return (
          <div>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--app-text)' }}>{menosRegistrosEv?.nombre || '—'}</div>
      ...
      case 'mejor_costo_reg':
        return (
          <div>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--app-text)' }}>{mejorCostoEv?.nombre || '—'}</div>
      ...
      case 'mayor_costo_reg':
        return (
          <div>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--app-text)' }}>{mayorCostoEv?.nombre || '—'}</div>
      ...
      case 'panel_mas_tickets':
        return (
          <div>
            ...
            <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--app-text)' }}>{panelEntry ? (PANEL_LABELS[panelEntry[0]] || panelEntry[0]) : '—'}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {estados.map(e => (
                <div key={e.key} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--app-text-2)', minWidth: 90 }}>{e.label}</span>
                  <div style={{ flex: 1, height: 6, background: 'var(--app-surface-2)', borderRadius: 99, overflow: 'hidden' }}>
                    {estadoCounts[e.key] > 0 && <div style={{ height: '100%', width: `${estadoCounts[e.key] / maxEstado * 100}%`, background: e.color, borderRadius: 99 }} />}
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--app-text)', minWidth: 20, textAlign: 'right' }}>{estadoCounts[e.key]}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {tiposConConteo.map(t => (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 13, color: 'var(--app-text-2)', minWidth: 90 }}>{t.label}</span>
                  <div style={{ flex: 1, height: 6, background: 'var(--app-surface-2)', borderRadius: 99, overflow: 'hidden' }}>
                    {t.count > 0 && <div style={{ height: '100%', width: `${t.count / maxT * 100}%`, background: t.color || '#e53e3e', borderRadius: 99 }} />}
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--app-text)', minWidth: 20, textAlign: 'right' }}>{t.count}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--app-text-subtle)', textTransform: 'uppercase', letterSpacing: 0.5 }}>Mejor Costo/Registro</span>
                </div>
                <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--app-text)' }}>{mejorCostoActivo?.nombre || '—'}</div>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--app-text-subtle)', textTransform: 'uppercase', letterSpacing: 0.5 }}>Más Registros</span>
                </div>
                <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--app-text)' }}>{masRegistrosActivo?.nombre || '—'}</div>
        ...
        return (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {topReg.length === 0 && <div style={{ fontSize: 13, color: 'var(--app-text-subtle)' }}>Sin datos</div>}
              {topReg.map((e, i) => (
                <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 11, color: 'var(--app-text-subtle)', width: 16, textAlign: 'right' }}>{i + 1}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--app-text-2)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{e.nombre}</div>
                    <div style={{ height: 4, background: 'var(--app-surface-2)', borderRadius: 99, overflow: 'hidden', marginTop: 3 }}>
                      <div style={{ height: '100%', width: `${(e.registrosActuales || 0) / maxTopReg * 100}%`, background: '#e53e3e', borderRadius: 99 }} />
                    </div>
                  </div>
                  <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--app-text)', minWidth: 46, textAlign: 'right' }}>{fmtN(e.registrosActuales)}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      case 'ranking_costo_reg':
        return (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {topCosto.length === 0 && <div style={{ fontSize: 13, color: 'var(--app-text-subtle)' }}>Sin datos</div>}
              {topCosto.map((e, i) => (
                <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 11, color: 'var(--app-text-subtle)', width: 16, textAlign: 'right' }}>{i + 1}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                        <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--app-text-2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '70%' }}>{e.nombre}</span>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--app-text-subtle)', minWidth: 52, textAlign: 'right' }}>{fmtN(e.registrosActuales)}</span>
                  </div>
                );
              })}
            </div>
      ...
      case 'uso_presupuesto':
        return (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {eventos.filter(e => (e.presupuestoTotal || 0) > 0).length === 0 && <div style={{ fontSize: 13, color: 'var(--app-text-subtle)' }}>Sin datos de presupuesto</div>}
              {eventos.filter(e => (e.presupuestoTotal || 0) > 0).map(e => {
                ...
                return (
                  <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--app-text-subtle)', minWidth: 52, textAlign: 'right' }}>{fmtMXN(e.presupuestoGastado)}</span>
                  </div>
                );
              })}
            </div>
      ...
      case 'comparativa': {
        ...
        function diasLabel(e) {
          ...
          if (d < 0)  return { text: 'Pasado', color: 'var(--app-text-subtle)' };
          ...
          <div>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {sorted.length === 0 && (
                    <tr><td colSpan={8} style={{ ...tdStyle, textAlign: 'center', color: 'var(--app-text-subtle)' }}>Sin eventos</td></tr>
                  )}
                  {sorted.map(e => {
                      ...
                      <tr key={e.id}>
                        <td style={tdStyle}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                            ...
                            <span style={{ fontWeight: 500, color: 'var(--app-text)' }}>{e.nombre}</span>
                          </div>
                        </td>
                        ...
                        <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700, color: 'var(--app-text)' }}>{fmtN(e.registrosActuales)}</td>
                        <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 600, color: 'var(--app-text-2)' }}>{fmtN(e.registrosMeta)}</td>
      ...
      default:
        return <div style={{ color: 'var(--app-text-subtle)', fontSize: 13 }}>Bloque no disponible</div>;
    }
  }
  ...
  return (
    <div style={{ background: 'var(--app-bg)', minHeight: '100%' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: winWidth < 600 ? '14px 16px 12px' : '20px 28px 16px', background: 'var(--app-surface)', borderBottom: '1px solid var(--app-border)', position: 'sticky', top: 0, zIndex: 20 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--app-text)', margin: 0 }}>Analíticas de Eventos</h1>
          <div style={{ fontSize: 12, color: 'var(--app-text-subtle)', marginTop: 3 }}>{eventos.length} eventos · {bloques.length} bloques activos</div>
        </div>
      ...
      {showPicker && (
        <div onClick={e => { if (e.target === e.currentTarget) setShowPicker(false); }}
          ...
          <div style={{ background: 'var(--app-surface)', borderRadius: 20, width: '100%', maxWidth: 560, maxHeight: '88vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>

            {/* Modal header */}
            <div style={{ padding: '22px 24px 16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                <span style={{ fontSize: 18, fontWeight: 700, color: 'var(--app-text)' }}>Agregar Bloque de Datos</span>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '0 12px 16px' }}>
              {bloquesFiltrados.length === 0 && (
                <div style={{ textAlign: 'center', padding: '30px', color: 'var(--app-text-subtle)', fontSize: 13 }}>Sin resultados</div>
              )}
